...
full_pnt_order<- read_csv(col_types = cols(), here("dev", "pnt-old", "data", "items.csv"))[14:189,] %>%
  mutate(n = row_number()) %>%
  select(item = target, n)
# who has a full test
full_test_id = read_csv(col_types = cols(), here("dev", "pnt-old", "validation", "validation.csv")) %>%
